...
void Simulation::file_writing(string filename)
{
	...
	for (size_t i(0); i < scavengers.size(); ++i)
	{
		file << "\t" << scavengers[i].get_lifeform_pos().x << " "
			 ...
			 << " ";
		if (scavengers[i].get_id_cible() != -1)
		{
			file << scavengers[i].get_id_cible();
		}
		file << endl;
	}
	file.close();
...
const int Simulation::algue_proche(size_t i)
{
	...
	for (size_t j(0); j < algae.size(); ++j)
	{
			...
			corals[i].get_cor_element(corals[i].get_cor_size() - 1).longueur)
		{
			...
			{
				if (vect_y >= 0 or (vect_y <= 0 and corals[i].get_cor_element(corals[i].get_cor_size() - 1).angle < 0))
				{
					new_angle = atan2(vect_y, vect_x) - corals[i].get_cor_element(corals[i].get_cor_size() - 1).angle;
				}
				else
				{
					new_angle = 2 * M_PI + atan2(vect_y, vect_x) -
								corals[i].get_cor_element(corals[i].get_cor_size() - 1).angle;
				}
			...
			{
				if ((vect_y >= 0 and corals[i].get_cor_element(corals[i].get_cor_size() - 1).angle > 0) or
					(vect_y <= 0))
				{
					new_angle = corals[i].get_cor_element(corals[i].get_cor_size() - 1).angle -
								atan2(vect_y, vect_x);
				}
				else
				{
					new_angle = 2 * M_PI + corals[i].get_cor_element(corals[i].get_cor_size() - 1).angle -
								atan2(vect_y, vect_x);
				}
				if (new_angle > 0 and new_angle < angle)
				{
...
const double Simulation::calcul_angle(size_t i, int j)
{
	...
	if (j != -1)
	{
		...
		if (corals[i].get_cor_dir() == Dir_rot_cor::TRIGO)
		{
			if (vect_y >= 0 or (vect_y <= 0 and corals[i].get_cor_element(corals[i].get_cor_size() - 1).angle < 0))
			{
				angle = atan2(vect_y, vect_x) - corals[i].get_cor_element(corals[i].get_cor_size() - 1).angle;
			}
			else
			{
		...
		else
		{
			if ((vect_y >= 0 and corals[i].get_cor_element(corals[i].get_cor_size() - 1).angle > 0) or
				(vect_y <= 0))
			{
				angle = corals[i].get_cor_element(corals[i].get_cor_size() - 1).angle -
						atan2(vect_y, vect_x);
			}
			else
			{
				angle = 2 * M_PI + corals[i].get_cor_element(corals[i].get_cor_size() - 1).angle -
						atan2(vect_y, vect_x);
			}
		}
	}
...
void Simulation::super_maj_cor(Coral& coral)
{

	/** TODO	 * 
	 * If length of last seg. > repro length
	 * 			-> reporduce
	 * 
	 * Else
	 * 			-> move
	 * 			-> find nearest algae
	 * 			-> eat if possible
	 * 			-> check if last segment intersects penultimate
	 * 			-> check if last segment intersects other segments
	 * 			-> check if last segment intersects other corals ; change rotation and cancel update
	 * 			-> check if within world borders ; change rotation and cancel update
	 */

	double dist = 0;

	Alga* closest = get_closest_alga(coral, &dist);

	if(alga) {
		double angle_step = 0;

		bool out_of_range_angle = get_angle_step(coral, *alga, angle_step);
		eat_alga(coral, *alga, dist, angle_step, out_of_range_angle);
	}

	bool should_reproduce = coral.maj_coral(closest, angle, dist);

	if(should_reproduce) {

		S2d tip = get_end_of_segment(coral.get_last_segment());
		Corail child(tip, 1, creation_id(), Status_cor::ALIVE, 
					  Dir_rot_cor::TRIGO, Status_dev::EXTEND, 1);

		child.add_segment(coral.get_last_segment().angle, l_repro - l_seg_interne);
		corals.emplace_back(corail);
	}

}

Alga* Simulation::get_closest_eatable_alga(const Coral& coral, double* dist){

	Alga* closest = nullptr;

	for(const auto& alga : algae){
		double d = get_distance(alga.get_lifeform_pos(), coral.get_last_segment());
...
bool Simulation::get_angle_step(const Coral& coral, const Alga& alga, double& angle) {

	Segment last_seg = coral.get_last_segment();
	S2d vect_to_alga = {alga.get_lifeform_pos().x - last_seg.x, alga.get_lifeform_pos().y - last_seg.y};

	// Use cauchy schwarz
	double cos_angle = dot_product(last_seg, vect_to_alga) / (last_seg.length * dist);
	double angle = acos2(cos_angle);
	bool out_of_range = (angle > delta_rot) || (angle < -delta_rot);

	angle = (angle <= delta_rot) ? angle : delta_rot;
	angle = (angle >= -delta_rot) ? angle : -delta_rot;
...
void Simulation::eat_alga(Coral& coral, Alga& alga, double dist, double angle, bool out_of_range_angle){
	...
	if(dist >= coral.get_last_segment().length){
		return;
	}

	// check if angle is actually too large
	if(out_of_range_angle) {
		return;
	}

	// Ok to eat, proceeds
	alga.set_lifeform_age(max_life_alg);
}
